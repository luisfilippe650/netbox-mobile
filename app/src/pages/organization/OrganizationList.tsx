...
export function OrganizationList({
  ...
  const updateItems = (update: (current: readonly OrganizationItem[]) => OrganizationItem[]) => {
  ...
  const addItem = (event: FormEvent<HTMLFormElement>) => {
    ...
    const nextNumber = organizationItems.reduce((largest, item) => {
      const number = Number(item.id.match(/\d+$/)?.[0] ?? 0)
      return Math.max(largest, number)
    }, 0) + 1
    const prefix = items[0]?.id.split('-')[0] ?? singular.slice(0, 3).toLocaleUpperCase('pt-BR')
  ...
  return (
    ...
    >
      ...
      <section className="organization__heading" aria-label={`Resumo de ${sectionTitle.toLocaleLowerCase('pt-BR')}`}>
        <div>
          <h2>{sectionTitle}</h2>
          <p>{filteredItems.length} {filteredItems.length === 1 ? singular.toLocaleLowerCase('pt-BR') : 'itens'} encontrado(s)</p>
        </div>
        <div className="organization__actions">
          <button className="organization__add" type="button" onClick={() => setShowAddForm(true)}>
            <span aria-hidden="true">+</span>
            Adicionar
          </button>
          {selectedIds.size > 0 ? (
            <button className="organization__delete" type="button" onClick={() => setShowDeleteConfirmation(true)}>
      ...
      {showAddForm ? (
        ...
        >
          <form className="organization__modal" onSubmit={addItem}>
            <span className="organization__modal-icon" aria-hidden="true">+</span>
            <h2>Adicionar {singular.toLocaleLowerCase('pt-BR')}</h2>
            <p>Preencha as informações do novo cadastro.</p>
            {isLocationPage ? (
              <label>
                <span>Site</span>
                <select required value={newSite} onChange={(event) => setNewSite(event.target.value)}>
                  <option value="">Selecione um site</option>
                  {siteOptions.map((site) => <option key={site.id} value={site.name}>{site.name}</option>)}
                </select>
              </label>
            ) : null}
            <label>
            ...
            {isDeviceFunctionPage ? (
              <label>
                ...
                <select value={newVmRole} onChange={(event) => setNewVmRole(event.target.value)}>
            ...
            <div className="organization__modal-actions">
              ...
              <button className="organization__confirm-add" type="submit">Adicionar</button>
            </div>
          </form>
        </div>
      ) : null}
      ...
      {selectedItem && hasDetails ? (
        ...
        >
          <section className="organization__modal organization__details" role="dialog" aria-modal="true" aria-labelledby="site-details-title">
            <span className="organization__modal-icon" aria-hidden="true">{isSitePage ? 'S' : 'L'}</span>
            <div>
              <h2 id="site-details-title">Informações do {singular.toLocaleLowerCase('pt-BR')}</h2>
              <p>{selectedItem.id}</p>
            </div>
            <dl>
              {isLocationPage ? (
            ...
            <button className="organization__details-close" type="button" onClick={() => setSelectedItem(null)}>
              Fechar
            </button>
          </section>
        </div>
      ...
      {showDeleteConfirmation ? (
        <div className="organization__modal-backdrop" role="presentation">
          <section className="organization__modal" role="alertdialog" aria-modal="true" aria-labelledby="organization-delete-title">
            <span className="organization__modal-icon organization__modal-icon--danger" aria-hidden="true">!</span>
            <h2 id="organization-delete-title">Excluir {selectedIds.size === 1 ? singular.toLocaleLowerCase('pt-BR') : 'itens'}?</h2>
            ...
            <div className="organization__modal-actions">
              ...
              <button className="organization__confirm-delete" type="button" onClick={deleteSelected}>Excluir</button>
            </div>
          </section>
        </div>
      ) : null}
