...
import type { DeviceRoleColor, NetBoxRackGroup, NetBoxRackRole } from "../../services";
...
export type RackCreateInput = { siteId: number; locationId: number | null; groupId: number | null; roleId: number | null; name: string; description: string; width: number; startingUnit: number; height: number };
type AddRackProps = { sites: readonly OrganizationItem[]; locations: readonly OrganizationItem[]; groups: readonly NetBoxRackGroup[]; roles: readonly NetBoxRackRole[]; onBack: () => void; onCreate: (input: RackCreateInput) => Promise<void>; onCreateGroup: (name: string) => Promise<number>; onCreateRole: (name: string, color: DeviceRoleColor) => Promise<number> };
const rackWidths = [10, 19, 21, 23] as const;

export default function AddRack({ sites, locations, groups, roles, onBack, onCreate, onCreateGroup, onCreateRole }: AddRackProps) {
  const { can } = useAccess();
  const [selectedSite, setSelectedSite] = useState(""); const [selectedLocation, setSelectedLocation] = useState("");
  const [showGroupForm, setShowGroupForm] = useState(false); const [newGroup, setNewGroup] = useState("");
  const [showRoleForm, setShowRoleForm] = useState(false); const [newRole, setNewRole] = useState("");
  const [selectedGroup, setSelectedGroup] = useState(""); const [selectedRole, setSelectedRole] = useState("");
  const [newRoleColor, setNewRoleColor] = useState<DeviceRoleColor>(defaultDeviceRoleColor);
  ...
  const createGroup = async () => {
    ...
    try { const id = await onCreateGroup(name); setSelectedGroup(String(id)); setNewGroup(""); setShowGroupForm(false); }
    catch (createError) { setError(createError instanceof Error ? createError.message : "Não foi possível criar o grupo de racks."); }
    finally { setIsSubmitting(false); }
  };
  const createRole = async () => {
    const name = newRole.trim(); if (!name) return;
    setIsSubmitting(true); setError("");
    try { const id = await onCreateRole(name, newRoleColor); setSelectedRole(String(id)); setNewRole(""); setNewRoleColor(defaultDeviceRoleColor); setShowRoleForm(false); }
    catch (createError) { setError(createError instanceof Error ? createError.message : "Não foi possível criar a função do rack."); }
    finally { setIsSubmitting(false); }
  };
  ...
  return <PageShell className="add-rack-page" eyebrow="Racks" title="Adicionar rack" subtitle="Preencha as informações do novo rack.">
    ...
    <form className="add-rack__form" onSubmit={(event) => void submit(event)}>
      <section className="add-rack__section"><div className="add-rack__section-title"><h2>Localização</h2><p>Defina onde o rack será instalado.</p></div>
        ...
        <div className="add-rack__related"><label className="add-rack__field"><span>Grupo de racks</span><select name="rackGroup" value={selectedGroup} onChange={(event) => setSelectedGroup(event.target.value)}><option value="">Sem grupo</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.name ?? group.display}</option>)}</select></label>{can("dcim.rackgroup", "add") ? <button className="add-rack__create-related" type="button" onClick={() => setShowGroupForm((current) => !current)}>+ Criar grupo de racks</button> : null}</div>
        {showGroupForm ? <div className="add-rack__quick-create"><label className="add-rack__field"><span>Novo grupo de racks</span><input autoFocus value={newGroup} onChange={(event) => setNewGroup(event.target.value)} placeholder="Ex.: Fileira principal" /></label><button type="button" disabled={isSubmitting || !newGroup.trim()} onClick={() => void createGroup()}>Adicionar grupo</button></div> : null}
        <div className="add-rack__related"><label className="add-rack__field"><span>Função do rack</span><select name="rackRole" value={selectedRole} onChange={(event) => setSelectedRole(event.target.value)}><option value="">Sem função</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.name ?? role.display}</option>)}</select></label>{can("dcim.rackrole", "add") ? <button className="add-rack__create-related" type="button" onClick={() => setShowRoleForm((current) => !current)}>+ Criar função do rack</button> : null}</div>
        {showRoleForm ? <div className="add-rack__quick-create"><label className="add-rack__field"><span>Nova função do rack</span><input autoFocus value={newRole} onChange={(event) => setNewRole(event.target.value)} placeholder="Ex.: Produção" /></label><DeviceRoleColorPicker value={newRoleColor} onChange={setNewRoleColor} disabled={isSubmitting} /><button type="button" disabled={isSubmitting || !newRole.trim()} onClick={() => void createRole()}>Adicionar função</button></div> : null}
      </section>
