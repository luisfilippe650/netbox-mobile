  ...
  return <PageShell className="add-rack-page" eyebrow="Racks" title="Adicionar rack" subtitle="Preencha as informações do novo rack.">
    ...
    <form className="add-rack__form" onSubmit={(event) => void submit(event)}>
      <section className="add-rack__section"><div className="add-rack__section-title"><h2>Localização</h2><p>Defina onde o rack será instalado.</p></div>
        ...
        <div className="add-rack__related"><label className="add-rack__field"><span>Função do rack</span><select name="rackRole" value={selectedRole} onChange={(event) => setSelectedRole(event.target.value)}><option value="">Sem função</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.name ?? role.display}</option>)}</select></label><button className="add-rack__create-related" type="button" onClick={() => setShowRoleForm((current) => !current)}>+ Criar função do rack</button></div>
