...
export default function DeviceTypes({ items, onAdd, onDelete, onBack }: DeviceTypesProps) {
  ...
  const deleteSelected = async () => {
    ...
    try {
      ...
      setError(deleteError instanceof Error ? deleteError.message : "Não foi possível excluir os tipos de dispositivos.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <PageShell className="organization-page" eyebrow="Dispositivos" title="Tipos de dispositivos" subtitle="Consulte e gerencie os modelos disponíveis no datacenter.">
      <label className="organization__search">
        <span className="organization__search-icon" aria-hidden="true" />
        <span className="organization__search-label">Tipos de dispositivos</span>
        <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por modelo ou fabricante" />
      </label>
      ...
      <div className="organization__list">
        {filteredItems.map((item) => {
          const manufacturer = item.manufacturer.name ?? item.manufacturer.display;
          return (
            <article
            ...
            >
              ...
              <div className="organization__card-content">
                ...
                <small><span>{manufacturer} · {item.u_height}U · {item.device_count} dispositivo(s)</span></small>
              </div>
            </article>
          );
        })}
      ...
      {filteredItems.length === 0 ? (
        <section className="organization__empty" role="status">
          <span aria-hidden="true">⌕</span><strong>Nenhum tipo de dispositivo encontrado</strong><p>Tente buscar usando outro modelo ou fabricante.</p>
        </section>
      ) : null}
      ...
      {selectedItem ? (
        <div className="organization__modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedItem(null); }}>
          <section className="organization__modal organization__details" role="dialog" aria-modal="true" aria-labelledby="device-type-details-title">
            <span className="organization__modal-icon" aria-hidden="true">T</span>
            <div><h2 id="device-type-details-title">Informações do tipo de dispositivo</h2><p>ID {selectedItem.id}</p></div>
            <dl>
              <div><dt>Modelo</dt><dd>{selectedItem.model}</dd></div>
              <div><dt>Fabricante</dt><dd>{selectedItem.manufacturer.name ?? selectedItem.manufacturer.display}</dd></div>
              <div><dt>Slug</dt><dd>{selectedItem.slug}</dd></div>
              <div><dt>Altura</dt><dd>{selectedItem.u_height}U</dd></div>
              <div><dt>Dispositivos</dt><dd>{selectedItem.device_count}</dd></div>
      ...
      {showDeleteConfirmation ? (
        <div className="organization__modal-backdrop" role="presentation">
          <section className="organization__modal" role="alertdialog" aria-modal="true" aria-labelledby="device-type-delete-title">
            <span className="organization__modal-icon organization__modal-icon--danger" aria-hidden="true">!</span>
            <h2 id="device-type-delete-title">Excluir tipo(s) de dispositivo?</h2>
