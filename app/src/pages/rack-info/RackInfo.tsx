...
export default function RackInfo({ onBack, onAdd, onDelete, onSelect, items }: RackInfoProps) {
  ...
  const deleteSelected = async () => {
    ...
    try {
      ...
      setError(deleteError instanceof Error ? deleteError.message : "Não foi possível excluir os racks.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    ...
    >
      <label className="organization__search">
        ...
        <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nome, ID ou local" />
      </label>
      {error ? <p className="rack-list__error" role="alert">{error}</p> : null}
      ...
      {filteredItems.length === 0 ? <p className="rack-list__empty">Nenhum rack encontrado.</p> : null}
      ...
      {showDeleteConfirmation ? (
        <div className="organization__modal-backdrop" role="presentation">
          <section className="organization__modal" role="alertdialog" aria-modal="true" aria-labelledby="rack-delete-title">
