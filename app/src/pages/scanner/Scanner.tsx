...
export default function Scanner({ onBack, onOpenDevice }: ScannerProps) {
  ...
  const [message, setMessage] = useState(
    "A câmera será usada somente para ler o QR code.",
  );
  ...
  return (
    <PageShell
      ...
      eyebrow="Scanner"
      title="Ler QR code"
      subtitle="Posicione o código dentro da área indicada."
    >
      <section className="scanner__panel" aria-live="polite">
        <div className="scanner__frame">
          <div className="scanner__topbar">
            ...
            <span>
              {status === "scanning" ? "Câmera ativa" : "Scanner de QR code"}
            </span>
          </div>
          <video
            ref={videoRef}
          ...
          {status === "starting" ? (
            <div className="scanner__state">Abrindo câmera…</div>
          ) : null}
          {status === "scanning" ? (
