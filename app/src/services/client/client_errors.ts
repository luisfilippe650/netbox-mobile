...
export function apiErrorMessage(payload: unknown, fallback: string): string {
  ...
  if (serialized.includes('unable to log in with provided credentials')) return 'Usuário ou senha inválidos.'
  if (serialized.includes('authentication credentials were not provided')) return 'Sua sessão não é válida. Entre novamente.'
