import { deviceRolesApi, devicesApi, deviceTypesApi, manufacturersApi } from './devices_api'
import type { NetBoxDevice, NetBoxDeviceRole, NetBoxManufacturer } from './devices_dto'
...
export function mapManufacturers(items: NetBoxManufacturer[]): OrganizationSummary[] {
...
export function mapDeviceRoles(items: NetBoxDeviceRole[]): OrganizationSummary[] {
  return items.map((item) => ({
    id: String(item.id),
    name: item.name ?? item.display,
    description: item.description || 'Sem descrição',
